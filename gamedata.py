...
def selection(*args):
    if args != ():
        for i in range(len(args)):
            UserObject.selection_list.append('{}) {}'.format(i + 1, args[i]))
    else:
        UserObject.selection_list.append('next')
...
def print_msg(word):
    UserObject.send_list.append(word)

class FunctionList():

    def start(self, choice):
        if choice == None:
            write("```{}```".format(pics[0]), isPic=True)
            ...
            print_msg('Чтобы выбрать вариант ответа, отправьте сообщение с соответствующей цифрой, без посторонних символов.')
        if choice == 1:
            UserObject.next_func = 'locked'
        if choice == 2:
            UserObject.next_func = 'iraida'
    ...
    def locked(self, choice):
        if choice == None:
            ...
            print_msg('Подсказка: хороший совет можно получить от игната, введя команду !игнат <текст вопроса>. Например: "!игнат синус".')
            selection('3.14', '0.42', '1.25')
        if choice == 1:
            write('Введенный код оказался верным, и замок со скрипом открылся.')
            UserObject.next_func = 'attic'
    ...
    def attic(self, choice):
        ...
        if choice == 1:
            UserObject.next_func = 'search'
        if choice == 2:
            UserObject.next_func = 'sit_on_the_steps'
    ...
    def search(self, choice):
        ...
        if choice == 1:
            UserObject.next_func = 'game_over'
        if choice == 2:
            UserObject.next_func = 'ouch'
    ...
    def bathroom(self, choice):
        ...
        if choice == 1:
            UserObject.next_func = 'experiment'
        if choice == 2:
            UserObject.next_func = 'find_a_hydrometer'
    ...
    def find_a_hydrometer(self, choice):
        ...
        if choice == 1:
            UserObject.next_func = 'game_over'
        if choice == 2:
            UserObject.next_func = 'bedroom'

    def bedroom(self, choice):
        if choice == None:
            write("```{}```".format(pics[2]), isPic=True)
    ...
    def hall(self, choice):
        if choice == None:
            ...
            write("```{}```".format(pics[3]), isPic=True)
    ...
    def magnet(self, choice):
        ...
        if choice == 1:
            ...
            UserObject.next_func = 'magnet'
        if choice == 2:
            UserObject.next_func = 'game_over2'
        if choice == 3:
            UserObject.next_func = 'water'
    ...
    def fortune(self, choice):
        ...
        if choice == 1:
            UserObject.next_func = 'phone_call'
        if choice == 2:
            UserObject.next_func = 'game_over3a'
        if choice == 3:
            UserObject.next_func = 'vitya2'
    ...
    def phone_call(self, choice):
        ...
        if choice == 1:
            UserObject.next_func = 'uncomfortable'
        if choice == 2:
            UserObject.next_func = 'sunday'
    ...
    def uncomfortable(self, choice):
        if choice == None:
            ...
            selection('Не отвечать на звонок', 'Ответить на звонок')
        if choice == 1:
            UserObject.next_func = 'uncomfortable2'
        if choice == 2:
            UserObject.next_func = 'sunday'
    ...
    def game_over3a(self, choice):
        if choice == None:
            ...
            UserObject.next_func = 'game_over3b'
